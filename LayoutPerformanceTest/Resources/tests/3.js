 ...
 *
 * Question:
 * Given n child views in a flat structure containing text of dimensions SIZE x SIZE, if we change the text of m <= n
 ...
 * 3) Add n views to the container at a randomized left and top position with randomized text
 *
 * Test operation:
 * 1) Per iteration:
 *		1) Randomly select m views to change the text of
...
function generateText() {
	var text = '',
		i;
	for (i = 0; i < Math.random() * CHARS_SPREAD + MIN_CHARS; i++) {
		text += String.fromCharCode(Math.round(Math.random() * 94 + 32));
	}
	return text;
}
...
function configureTest(name) {
	...
	inputValidators.push({
		inputField: createEntry('Min Characters', MIN_CHARS),
		parseValue: function () {
			var value = parseInt(this.inputField.value, 10);
			if (!isNaN(value) && value > 0) {
				MIN_CHARS = value;
				console.log('Running test with ' + MIN_CHARS + ' minimum characters per child');
			} else {
				console.warn('Invalid minimum number of characters specified, using the default of ' + MIN_CHARS);
			}
		}
	});
	inputValidators.push({
		inputField: createEntry('Character Spread', CHARS_SPREAD),
		parseValue: function () {
			var value = parseInt(this.inputField.value, 10);
			if (!isNaN(value) && value > 0) {
				CHARS_SPREAD = value;
				console.log('Running test with a character spread of ' + CHARS_SPREAD);
			} else {
				console.warn('Invalid character spread specified, using the default of ' + CHARS_SPREAD);
			}
		}
	});
	inputValidators.push({
	...
	if (iteration <= NUM_ITERATIONS) {
		setTimeout(function () {
			...
			for (i = 0; i < NUM_CHILDREN_TO_CHANGE; i++) {
				...
				do {
					text = generateText();
				} while(text.length === child.text.length);
				child.text = text;
